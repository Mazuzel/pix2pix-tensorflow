...
parser.add_argument("--nef", type=int, default=64, help="number of generator filters in encoder first conv layer")
parser.add_argument("--ngf", type=int, default=64, help="number of generator filters in decoder first conv layer")
...
def load_examples():
    ...
    def transform(image, isDepthImage):
        ...
        r = tf.image.resize_images(r, [scale, scale], method=tf.image.ResizeMethod.AREA)

        offset = tf.cast(tf.floor(tf.random_uniform([2], 0, scale - CROP_SIZE + 1, seed=seed)), dtype=tf.int32)
...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    # encoder_1: [batch, 256, 256, in_channels] => [batch, 128, 128, nef]
    with tf.variable_scope("encoder_1"):
        output = conv(generator_inputs, a.nef, stride=2)
        layers.append(output)

    layer_specs = [
        a.nef * 2, # encoder_2: [batch, 128, 128, nef] => [batch, 64, 64, nef * 2]
        a.nef * 4, # encoder_3: [batch, 64, 64, nef * 2] => [batch, 32, 32, nef * 4]
        a.nef * 8, # encoder_4: [batch, 32, 32, nef * 4] => [batch, 16, 16, nef * 8]
        a.nef * 8, # encoder_5: [batch, 16, 16, nef * 8] => [batch, 8, 8, nef * 8]
        a.nef * 8, # encoder_6: [batch, 8, 8, nef * 8] => [batch, 4, 4, nef * 8]
        a.nef * 8, # encoder_7: [batch, 4, 4, nef * 8] => [batch, 2, 2, nef * 8]
        a.nef * 8, # encoder_8: [batch, 2, 2, nef * 8] => [batch, 1, 1, nef * 8]
    ]
    ...
    layer_specs = [
        ...
        (a.ngf * 4, 0.0),   # decoder_5: [batch, 8, 8, ngf * 8 * 2] => [batch, 16, 16, ngf * 4 * 2]
        (a.ngf * 4, 0.0),   # decoder_4: [batch, 16, 16, ngf * 4 * 2] => [batch, 32, 32, ngf * 4 * 2]
...
def main():
    ...
    if a.mode == "test" or a.mode == "export":
        ...
        options = {"which_direction", "nef", "ngf", "ndf", "lab_colorization"}
